    ...
    $.fn.scannerDetection = function (options) {
        ...
        this.each(function () {
            ...
            /*self.scannerDetectionTest = function (s) {
                ...
                console.log("false"+scanInProgress)
                // Set the flag to indicate scanning is in progress
                scanInProgress = true; // myself added
                console.log("true"+scanInProgress)


                // If all conditions are good (length, time...), call the callback and re-initialize the plugin for next scanning
                ...
                if (stringWriting.length >= options.minLength && lastCharTime - firstCharTime < stringWriting.length * options.avgTimeByChar) {
                    ...
                    // If the currently focused element is an input field, clear its value
                    var focusedElement = document.activeElement;
                    if (focusedElement.tagName === 'INPUT' || focusedElement.tagName === 'TEXTAREA') {
                        focusedElement.value = '';
                    }
                    // Reset the flag after scanning is complete
                    scanInProgress = false; //myself added
                    ...
                    initScannerDetection();
                    // Reset the flag after scanning is complete (in case of an error)
                    scanInProgress = false;  //myself added
